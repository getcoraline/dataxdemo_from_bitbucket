...
with dbt_internal_unit_test_actual as (
  select
    customer_id,total_orders,total_spent,avg_order_value,customer_segment, 'actual' as `actual_or_expected`
  from (
...
dbt_internal_unit_test_expected as (
  select
    customer_id, total_orders, total_spent, avg_order_value, customer_segment, 'expected' as `actual_or_expected`
  from (
    select cast(1 as string)
 as customer_id, cast(2 as bigint)
 as total_orders, cast(3500 as bigint)
 as total_spent, cast(1750 as double)
 as avg_order_value, cast('Regular' as string)
 as customer_segment
union all
select cast(2 as string)
 as customer_id, cast(1 as bigint)
 as total_orders, cast(800 as bigint)
 as total_spent, cast(800 as double)
 as avg_order_value, cast('New' as string)
 as customer_segment
